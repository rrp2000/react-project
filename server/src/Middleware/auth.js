...
const authentication = function (req, res, next) {
  try {
    ...
    if (!token) return res.status(401).send({ status: false, message: "token must be present" });    //If neither condition satisfies & no token is present in the request header return error
